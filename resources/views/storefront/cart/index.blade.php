...
@section('content')
    ...
    @if ($cart->activeItems->isEmpty() && $cart->savedItems->isEmpty())
        <div class="bg-white shadow rounded-lg p-6 text-sm text-gray-500">
            Your cart is empty. Browse the <a href="{{ route('shop.index') }}" class="text-brand-orange hover:underline">shop</a> to add products.
        </div>
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="divide-y divide-gray-100">
                            @foreach ($items as $item)
                                <div class="p-4 flex gap-4">
                                    ...
                                    <div class="flex-1">
                                        <a href="{{ route('products.show', $item->product) }}" class="font-medium text-gray-900 hover:text-brand-orange">
                                            {{ $item->product->name }}
                                        </a>
                                        @if ($item->variation)
                                        ...
                                        @elseif ($item->hasPriceDrifted())
                                            <p class="mt-1 text-xs text-amber-600">
                                                Price changed to @price($item->currentUnitPrice()) since you added this.
                                            </p>
                                        @endif
                                        ...
                                        <div class="mt-2 flex items-center gap-4">
                                            <form method="POST" action="{{ route('cart.items.update', $item) }}" class="flex items-center gap-2">
                                                ...
                                                <button type="submit" class="text-xs text-brand-orange hover:underline">Update</button>
                                            </form>
                                ...
                                <div class="p-4 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <form method="POST" action="{{ route('cart.items.move-to-cart', $item) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-xs text-brand-orange hover:underline">Move to cart</button>
                                        </form>
                                        <form method="POST" action="{{ route('cart.items.destroy', $item) }}">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="bg-white shadow rounded-lg p-6 h-fit space-y-4">
                ...
                @if ($cart->activeItems->isNotEmpty())
                    <a href="{{ route('checkout.index') }}" class="block text-center rounded-md bg-brand-orange px-4 py-2 text-white text-sm font-medium hover:bg-brand-orange2">
                        Proceed to checkout
                    </a>
                @endif
            </div>
